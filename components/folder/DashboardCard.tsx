...
export function DashboardCard({ link }: DashboardCardProps) {
  const trendConfig = link.kpiTrend ? getTrendConfig(link.kpiTrend) : null;

  return (
    <Link
      href={`/dashboard/${link.id}`}
      className="flex h-full flex-col rounded-xl border border-gray-200 bg-white p-6 transition-shadow hover:shadow-md"
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <h3 className="text-lg font-bold text-gray-900">{link.title}</h3>
        <BarChart2 className="h-5 w-5 shrink-0 text-gray-500" />
      </div>

      <p className="line-clamp-2 text-sm text-gray-500">{link.description ?? "Sin descripción."}</p>

      {link.kpiValue ? (
        <div className="mt-auto pt-6">
          <div className="flex items-center gap-2">
            <p className="text-3xl font-bold text-gray-900">{link.kpiValue}</p>
            {link.kpiTrend ? (
              <span
                className={`inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-sm font-medium ${trendConfig?.classes ?? getTrendClasses(link.kpiTrend)}`}
              >
