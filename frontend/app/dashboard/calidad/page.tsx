  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center gap-4 py-20 text-slate-500">
        <p className="text-base font-medium">Error al cargar los datos de calidad.</p>
      </div>
    );
  }

  const kpis = byArea('calidad') as unknown as KPIItem[];

  if (kpis.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-400">
        <p className="text-4xl mb-3">🛠️</p>
        <p className="font-semibold text-slate-600">Sin indicadores de calidad configurados</p>
